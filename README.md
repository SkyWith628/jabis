...
| 문제 | 고민 | 해결 |
|---|---|---|
| **응답 지연** — 음성 비서는 STT→LLM→TTS 파이프라인이 길어 응답이 늦으면 대화가 어색 | 체감 지연을 어떻게 줄이나 | 대화 API를 **스트리밍**으로 구성해 첫 응답이 빨리 나오게 하고, 생성되는 대로 TTS로 읽어줌 |
| **이종 스택 통합** — 데스크탑(Electron)·UI(React)·AI(Python)를 하나로 묶어야 함 | 파이썬 AI 로직을 데스크탑 앱에 어떻게 붙이나 | **FastAPI를 로컬 서버**로 두고 Electron이 React UI를 띄워 호출하는 구조 — AI/백엔드와 UI의 책임을 분리 |
| **음성 입출력 품질** — 브라우저 STT는 쉽지만 한국어 TTS 품질이 아쉬움 | 입력과 출력을 각각 어디서 처리하나 | 입력은 브라우저 **Web Speech API(STT)**, 출력은 서버의 **Edge TTS**로 분리해 자연스러운 한국어 음성 확보 |
